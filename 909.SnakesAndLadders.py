...
class Solution:
    ...
    e.g. label 7 pos is [0, 0]. this method is a way to get row, col of a label
    """

    def __label_to_pos(self, label: int, n: int):
    ...
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        while queue:
            ...
            r, c = self.__label_to_pos(label, n)
            # replace label to destination, whether its ladder or snake
            if board[r][c] != -1:
                label = board[r][c]
            # break early if we reach the end
            if label == n * n:
                return move
            # role the dice to get every possible result
            for i in range(1, 7):
                new_label = label + i
                if new_label <= n * n and new_label not in visited:
                    visited.add(new_label)
                    queue.append((new_label, move + 1))
        return -1
